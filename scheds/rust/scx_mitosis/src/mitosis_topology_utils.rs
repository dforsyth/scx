...
impl MitosisTopology {
    ...
    pub fn apply(&self, skel: &mut OpenBpfSkel) -> Result<()> {
        ...
        self.apply_arrays(&mut bss.cpu_to_llc, &mut bss.llc_to_cpus)
    }
...
mod tests {
    ...
    fn test_topology() {
        ...
        for llc in 0..MAX_LLCS {
            ...
            assert!(check_llc(&llc_to_cpus, llc, first_cpu));
            assert!(check_llc(&llc_to_cpus, llc, last_cpu));

            if llc > 0 {
                assert!(!check_llc(&llc_to_cpus, llc, first_cpu - 1));
            }
            if llc + 1 < MAX_LLCS {
